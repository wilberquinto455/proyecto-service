    ...
    if (empty($Cod_producto) || !is_numeric($Cod_producto)) {
        ...
        if(in_array($imgExt, $valid_extensions)){
            ...
            if ($imgSize < 5000000) {
                ...
                $result = $objetoConsultas->updateProductoVendedor($Cod_producto, $Nombre_producto, $Descripcion_producto, $Marca, $Nit_proveedor, $Precio_producto, $imgProd);
            } else {
        ...
        // si no se realiza la comprobacion de la foto, es decir no se cargo ningun archivo
        // sucede lo mismo, pero sin mover la imagen porque no hay
    } else {
        if (empty($Cod_producto) || !is_numeric($Cod_producto)) {
            $_SESSION['titulo'] = "Oops!";
            $_SESSION['msj'] = "Por favor escriba un codigo del producto valido.";
